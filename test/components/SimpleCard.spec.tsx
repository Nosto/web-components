...
describe("SimpleCard", () => {
  ...
  const mockProduct = {
    ...
    vendor: "Test Brand",
    tags: ["test", "awesome"],
    price: 1999, // $19.99 in cents (product-level price)
    compare_at_price: 2499, // $24.99 in cents (product-level compare price)
  ...
  it("should not render discount when product has no discount", async () => {
    const productWithoutDiscount = {
      ...mockProduct,
      compare_at_price: null, // no compare price
      variants: [
        {
          id: 789,
          price: 1999,
  ...
  it("should use product-level pricing instead of variant-level", async () => {
    const productWithDifferentLevels = {
      ...mockProduct,
      price: 1599, // $15.99 product level
      compare_at_price: 1999, // $19.99 product level
      variants: [
        {
          id: 789,
          price: 2999, // $29.99 variant level - should be ignored
          compare_at_price: 3999, // $39.99 variant level - should be ignored
          available: true,
          title: "Default Title"
        }
      ]
    ...
    addProductHandlers({
      "test-product": { product: productWithDifferentLevels }
    })
    ...
    // Should use product-level pricing, not variant-level
    expect(card.innerHTML).toContain("$15.99") // product price
    expect(card.innerHTML).toContain("$19.99") // product compare_at_price
    expect(card.innerHTML).not.toContain("$29.99") // variant price should not appear
    expect(card.innerHTML).not.toContain("$39.99") // variant compare_at_price should not appear
  })
})