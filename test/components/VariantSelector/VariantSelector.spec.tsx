...
describe("VariantSelector", () => {
  ...
  it("should mark unavailable option values with unavailable part attribute", async () => {
    ...
    // Medium and Blue should be marked as unavailable
    const mediumButton = shadowRoot.querySelector(
      '[data-option-name="Size"][data-option-value="Medium"]'
    ) as HTMLButtonElement
    const blueButton = shadowRoot.querySelector(
    ...
    // Small, Large, and Red should be available
    const smallButton = shadowRoot.querySelector(
      '[data-option-name="Size"][data-option-value="Small"]'
    ) as HTMLButtonElement
    const largeButton = shadowRoot.querySelector(
      '[data-option-name="Size"][data-option-value="Large"]'
    ) as HTMLButtonElement
    const redButton = shadowRoot.querySelector(
      '[data-option-name="Color"][data-option-value="Red"]'
    ) as HTMLButtonElement

    expect(mediumButton.getAttribute("part")).toContain("unavailable")
    expect(blueButton.getAttribute("part")).toContain("unavailable")
    expect(smallButton.getAttribute("part")).not.toContain("unavailable")
    expect(largeButton.getAttribute("part")).not.toContain("unavailable")
    expect(redButton.getAttribute("part")).not.toContain("unavailable")
  })
  ...
  it("should apply unavailable part attribute enabling pointer-events: none via CSS", async () => {
    ...
    // Verify unavailable buttons have the unavailable part attribute
    // This enables the CSS rule: .value[part="value unavailable"] { pointer-events: none; }
    // which prevents clicks in real browsers (jsdom doesn't enforce pointer-events CSS)
    expect(mediumButton.getAttribute("part")).toBe("value unavailable")
    expect(blueButton.getAttribute("part")).toBe("value unavailable")

    // Available button should not have unavailable part
    expect(smallButton.getAttribute("part")).toContain("value")
    expect(smallButton.getAttribute("part")).not.toContain("unavailable")
  })
})
