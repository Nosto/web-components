...
describe("NostoImage/transform", () => {
  ...
  describe("alt and sizes attribute handling", () => {
    ...
    it("handles missing alt and sizes attributes", () => {
      const result = transform({ src: shopifyUrl, width: 400, height: 300 })
      expect(result.alt).toBeUndefined()
      expect(result.sizes).toBeDefined() // unpic generates default sizes
    })

    it("overrides unpic-generated sizes when custom sizes provided", () => {
      const customSizes = "(min-width: 1024px) 25vw, 50vw"
      const result = transform({
        src: shopifyUrl,
        width: 800,
        aspectRatio: 1.5,
