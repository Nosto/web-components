...
describe("image/transform", () => {
  it("returns original url and props for unknown provider", () => {
    ...
    expect(result.props.src).toBe(unknownUrl)
  ...
  describe("shopify transform", () => {
    ...
    it("transforms shopify url and returns correct props and style", () => {
      ...
      expect(result.props.src).toBe(`${baseUrl}image.jpg?width=400&height=200&crop=center`)
      expect(result.props.src).toContain("shopify")
      expect(result.props.width).toBeUndefined()
      expect(result.props.height).toBeUndefined()
      expect(result.props.crop).toBe("center")
      expect(typeof result.style).toBe("object")
    })

    it("handles missing width/height/crop gracefully", () => {
      const result = transform({ src: imageUrl })
      expect(result.props.src).toBe(`${baseUrl}image.jpg?width=200&height=300`)
      expect(result.props.width).toBeUndefined()
      expect(result.props.height).toBeUndefined()
      expect(result.props.src).toContain("shopify")
    ...
    it("sanitizes style and props (removes style key from props)", () => {
      ...
      expect(result.props.src).toBe(`${baseUrl}image.jpg?width=100&height=200`)
      expect(result.props.width).toBeUndefined()
      expect(result.props.height).toBeUndefined()
    ...
    it("preserves existing query params", () => {
      ...
      expect(result.props.src).toBe(`${baseUrl}image.jpg?foo=bar&baz=qux&width=120&height=80`)
      expect(result.props.src).toContain("foo=bar")
      expect(result.props.src).toContain("baz=qux")
      expect(result.props.src).toContain("width=120")
      expect(result.props.src).toContain("height=80")
    })
    ...
    it("prioritizes provided width/height over named dimensions", () => {
      ...
      expect(result.props.src).toBe(`${baseUrl}image.jpg?width=500&height=400`)
      expect(result.props.width).toBeUndefined()
      expect(result.props.height).toBeUndefined()
    })
    ...
    it("handles images with no dimension or crop", () => {
      ...
      expect(result.props.src).toContain("image.jpg")
    })
  })
