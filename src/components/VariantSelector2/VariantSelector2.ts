...
import { fetchProductFromStorefront } from "@/utils/shopifyStorefront"

const setShadowContent = shadowContentFactory(styles)

let placeholder = ""

/** Event name for the VariantSelector2 rendered event */
const VARIANT_SELECTOR_RENDERED_EVENT = "@nosto/VariantSelector2/rendered"

/**
 * A custom element that displays product variant options as clickable pills.
 *
 * Fetches product data from Shopify Storefront GraphQL API and renders option rows with
 ...
 * @property {string} storefrontAccessToken - Shopify Storefront API access token. Required.
 * @property {string} shopDomain - Shopify shop domain (e.g., 'myshop.myshopify.com'). Required.
 * @property {string} apiVersion - Shopify Storefront API version (defaults to '2024-01'). Optional.
 ...
 * @fires @nosto/VariantSelector2/rendered - Emitted when the component has finished rendering
 */
@customElement("nosto-variant-selector2")
export class VariantSelector2 extends NostoElement {
  @property(String) handle!: string
  @property(String) storefrontAccessToken!: string
  @property(String) shopDomain!: string
  @property(String) apiVersion?: string
  ...
  async connectedCallback() {
    ...
    assertRequired(this, "storefrontAccessToken")
    assertRequired(this, "shopDomain")
...
async function loadAndRenderMarkup(element: VariantSelector2) {
...
function initializeDefaultSelections(element: VariantSelector2, product: ShopifyProduct) {
...
function setupOptionListeners(element: VariantSelector2) {
...
export async function selectOption(element: VariantSelector2, optionName: string, value: string) {
...
function updateActiveStates(element: VariantSelector2) {
...
function updateUnavailableStates(element: VariantSelector2, product: ShopifyProduct) {
...
function emitVariantChange(element: VariantSelector2, product: ShopifyProduct) {
...
export function getSelectedVariant(element: VariantSelector2, product: ShopifyProduct): ShopifyVariant | null {
  return (
...
async function fetchProductData(element: VariantSelector2) {
  const { handle, storefrontAccessToken, shopDomain, apiVersion, filtered } = element

  const data = await fetchProductFromStorefront(handle, {
...
declare global {
  interface HTMLElementTagNameMap {
    "nosto-variant-selector2": VariantSelector2
  }
}
