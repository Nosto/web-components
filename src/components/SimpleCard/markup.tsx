...
import { generateCarouselHTML } from "./carousel.tsx"
...
function generateImageHTML(element: SimpleCard, title: string, images: ShopifyImage[]) {
  ...
  return (
    <div class={`image ${hasAlternate ? "alternate" : ""}`} part="image">
      ...
      {hasAlternate && alternateImage && generateImgHtml(alternateImage, title, "img alternate", element.imageSizes)}
    </div>
  )
}
...
export function generateImgHtml(image: ShopifyImage, alt: string, className: string, sizes?: string) {
  ...
  const filteredProps = Object.fromEntries(Object.entries(props).filter(([, value]) => value != null))
  return (
    <img
      alt={alt}
      part={className}
      ...
      style={style as Partial<CSSStyleDeclaration>}
      {...filteredProps}
    />
  )
}
