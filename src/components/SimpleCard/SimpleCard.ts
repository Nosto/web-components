...
import { generateCardTemplate } from "./markup"
 ...
 * Uses lit-html for templating instead of string-based HTML generation.
 *
 ...
 * @example
 * ```html
 * <nosto-simple-card handle="awesome-product" alternate brand discount rating="4.2"></nosto-simple-card>
 * ```
 */
...
async function loadAndRenderMarkup(element: SimpleCard) {
  ...
  const template = generateCardTemplate(element, productData)
